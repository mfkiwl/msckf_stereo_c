...
namespace cg {
    ...
    cg::Vector3 IMUState::gravity = cg::Vector3({0, 0, -GRAVITY_ACCELERATION});
    cg::EuclideanTransform IMUState::T_imu_body = cg::EuclideanTransform();

    // Static member variables in CAMState class.
    cg::EuclideanTransform CAMState::T_cam0_cam1 = cg::EuclideanTransform();
    ...
    bool MsckfVio::loadParameters() {
        ...
        state_server.imu_state.velocity = cfg_msckfvio["initial_state/velocity"].as<cg::Vector3>();
        ...
        state_server.state_cov = cg::Matrix(21, 21);
        for (int i = 3; i < 6; ++i)
        ...
        cg::Matrix m4_cam0_imu = cfg_cam_imu_["cam0"]["T_cam_imu"].as<cg::Mat4>();

        cg::EuclideanTransform T_cam0_imu = m4_cam0_imu.inv();
        state_server.imu_state.R_imu_cam0 = T_cam0_imu.rotation_matrix().transpose();
        state_server.imu_state.t_cam0_imu = T_cam0_imu.translation();

        cg::Matrix m4_cam1_cam0 = cfg_cam_imu_["cam1"]["T_cn_cnm1"].as<cg::Mat4>();
        CAMState::T_cam0_cam1 = m4_cam1_cam0; // 坐标系变换

        cg::Matrix m4_imu_body = cfg_cam_imu_["T_imu_body"].as<cg::Mat4>();
    ...
    bool MsckfVio::initialize() {
        ...
        state_server.continuous_noise_cov = cg::Matrix(12, 12);
        state_server.continuous_noise_cov.set_mat(0, 0, cg::Matrix::eye(3) * IMUState::gyro_noise);
        state_server.continuous_noise_cov.set_mat(3, 3, cg::Matrix::eye(3) * IMUState::gyro_bias_noise);
        state_server.continuous_noise_cov.set_mat(6, 6, cg::Matrix::eye(3) * IMUState::acc_noise);
        state_server.continuous_noise_cov.set_mat(9, 9, cg::Matrix::eye(3) * IMUState::acc_bias_noise);

        // Initialize the chi squared test table with confidence level 0.95.
        for (int i = 1; i < 100; ++i) {
//            boost::math::chi_squared chi_squared_dist(i);
//            chi_squared_test_table[i] = boost::math::quantile(chi_squared_dist, 0.05);
            chi_squared_test_table[i] = cg::chi_square_table_p95[i-1];
        }

        return true;
    }

    void MsckfVio::imuCallback(const cg::ImuConstPtr &msg) {
    ...
    void MsckfVio::initializeGravityAndBias() {
        ...
        cg::Vector3 sum_angular_vel;
        cg::Vector3 sum_linear_acc;

        for (const auto &imu_msg : imu_msg_buffer) {
            cg::Vector3 angular_vel = imu_msg.angular_velocity;
            cg::Vector3 linear_acc  = imu_msg.linear_acceleration;
        ...
        cg::Vector3 gravity_imu = sum_linear_acc / imu_msg_buffer.size();

        // Initialize the initial orientation, so that the estimation
        // is consistent with the inertial frame.
        double gravity_norm = gravity_imu.l2norm();
        IMUState::gravity = cg::Vector3({0.0, 0.0, -gravity_norm});

        std::cout << "===g: " << gravity_norm << std::endl;

        // TODO: verify
        state_server.imu_state.orientation = cg::from_two_vector(gravity_imu, -IMUState::gravity).transpose().quarternion();
...
//        imu_sub.shutdown();
        ...
        imu_state.time = 0.0;
        imu_state.orientation = cg::Quarternion();
        imu_state.position = cg::Vector3();
        imu_state.velocity = cg::Vector3();
        imu_state.gyro_bias = cg::Vector3();
        imu_state.acc_bias = cg::Vector3();
        imu_state.orientation_null = cg::Quarternion();
        imu_state.position_null = cg::Vector3();
        imu_state.velocity_null = cg::Vector3();
...
//        feature_sub = nh.subscribe("features", 40, &MsckfVio::featureCallback, this);
    ...
    void MsckfVio::processModel(const double &time, const cg::Vector3 &m_gyro, const cg::Vector3 &m_acc) {
        // Remove the bias from the measured gyro and acceleration
        IMUState &imu_state = state_server.imu_state;
        cg::Vector3 gyro = m_gyro - imu_state.gyro_bias;
        cg::Vector3 acc  = m_acc -  imu_state.acc_bias;
        ...
        F.set_mat(0,  0, -cg::skew_symmetric(gyro));
        F.set_mat(0,  3, -cg::Matrix::eye(3));
        F.set_mat(6,  0, -imu_state.orientation.rotation_matrix().transpose() * cg::skew_symmetric(acc));
        F.set_mat(6,  9, -imu_state.orientation.rotation_matrix().transpose());
        F.set_mat(12, 6,  cg::Matrix::eye(3));

        G.set_mat(0, 0, -cg::Matrix::eye(3));
        G.set_mat(3, 3,  cg::Matrix::eye(3));
        G.set_mat(6, 6, -imu_state.orientation.rotation_matrix().transpose());
        G.set_mat(9, 9,  cg::Matrix::eye(3));

        // Approximate matrix exponential to the 3rd order,
        // which can be considered to be accurate enough assuming dtime is within 0.01s.
        cg::Matrix Fdt = F * dtime;
        cg::Matrix Fdt_square = Fdt * Fdt;
        cg::Matrix Fdt_cube = Fdt_square * Fdt;
        cg::Matrix Phi = cg::Matrix::eye(21) + Fdt + 0.5 * Fdt_square + (1.0 / 6.0) * Fdt_cube;

        // Propogate the state using 4th order Runge-Kutta
        predictNewState(dtime, gyro, acc);

        // Modify the transition matrix
        cg::RotationMatrix R_kk_1 = imu_state.orientation_null.rotation_matrix();
        Phi.set_mat(0, 0, imu_state.orientation.rotation_matrix() * R_kk_1.transpose());

        cg::Vector3 u = R_kk_1 * IMUState::gravity;
        cg::Vector3 s = (1.0 / u.dot(u)) * u;

//        cg::Matrix A1 = Phi.block<3, 3>(6, 0);
        cg::Matrix A1 = Phi.block<3, 3>(6, 0);
        cg::Vector3 w1 = cg::skew_symmetric(imu_state.velocity_null - imu_state.velocity) * IMUState::gravity;
        Phi.set_mat(6, 0, A1 - (A1 * u - w1) * s.transpose());

        cg::Matrix A2 = Phi.block<3, 3>(12, 0);
        cg::Vector3 w2 = cg::skew_symmetric(dtime * imu_state.velocity_null + imu_state.position_null - imu_state.position) * IMUState::gravity;
        Phi.set_mat(12, 0, A2 - (A2 * u - w2) * s.transpose());

        // Propogate the state covariance matrix.
        cg::Matrix Q = Phi * G * state_server.continuous_noise_cov * G.transpose() * Phi.transpose() * dtime;
        ...
        cg::Matrix state_cov_fixed = (state_server.state_cov + state_server.state_cov.transpose()) * 0.5;
    ...
    void MsckfVio::predictNewState(const double &dt, const cg::Vector3 &gyro, const cg::Vector3 &acc) {
        ...
        cg::Matrix Omega(4, 4);
        Omega.set_mat(0, 0, -cg::skew_symmetric(gyro));
        Omega.set_mat(0, 3,  gyro);
        Omega.set_mat(3, 0, -gyro.transpose());

        cg::Quarternion &q = state_server.imu_state.orientation;
        cg::Vector3 &v = state_server.imu_state.velocity;
        cg::Vector3 &p = state_server.imu_state.position;

        // Some pre-calculation
        cg::Quarternion dq_dt, dq_dt2;
        if (gyro_norm > 1e-5) {
            dq_dt  = cg::Vector4((cos(gyro_norm * dt * 0.5)  * cg::Matrix::eye(4) + 1 / gyro_norm * sin(gyro_norm * dt * 0.5)  * Omega) * q.vector4());
            dq_dt2 = cg::Vector4((cos(gyro_norm * dt * 0.25) * cg::Matrix::eye(4) + 1 / gyro_norm * sin(gyro_norm * dt * 0.25) * Omega) * q.vector4());
        } else {
            dq_dt  = cg::Vector4((cg::Matrix::eye(4) + 0.5  * dt * Omega) * cos(gyro_norm * dt * 0.5)  * q.vector4());
            dq_dt2 = cg::Vector4((cg::Matrix::eye(4) + 0.25 * dt * Omega) * cos(gyro_norm * dt * 0.25) * q.vector4());
        }
        cg::RotationMatrix dR_dt_transpose  =  dq_dt.rotation_matrix().transpose();
        cg::RotationMatrix dR_dt2_transpose = dq_dt2.rotation_matrix().transpose();

        // k1 = f(tn, yn)
        cg::Vector3 k1_v_dot = q.rotation_matrix().transpose() * acc + IMUState::gravity;
        cg::Vector3 k1_p_dot = v;

        // k2 = f(tn+dt/2, yn+k1*dt/2)
        cg::Vector3 k1_v = v + k1_v_dot * dt / 2;
        cg::Vector3 k2_v_dot = dR_dt2_transpose * acc + IMUState::gravity;
        cg::Vector3 k2_p_dot = k1_v;

        // k3 = f(tn+dt/2, yn+k2*dt/2)
        cg::Vector3 k2_v = v + k2_v_dot * dt / 2;
        cg::Vector3 k3_v_dot = dR_dt2_transpose * acc + IMUState::gravity;
        cg::Vector3 k3_p_dot = k2_v;

        // k4 = f(tn+dt, yn+k3*dt)
        cg::Vector3 k3_v = v + k3_v_dot * dt;
        cg::Vector3 k4_v_dot = dR_dt_transpose * acc + IMUState::gravity;
        cg::Vector3 k4_p_dot = k3_v;
    ...
    void MsckfVio::stateAugmentation(const double &time) {
        const cg::RotationMatrix &R_i_c = state_server.imu_state.R_imu_cam0;
        const cg::Vector3 &t_c_i = state_server.imu_state.t_cam0_imu;

        // Add a new camera state to the state server.
        cg::RotationMatrix R_w_i = state_server.imu_state.orientation.rotation_matrix();
        cg::RotationMatrix R_w_c = R_i_c * R_w_i;
        cg::Vector3 t_c_w = state_server.imu_state.position + R_w_i.transpose() * t_c_i;
        ...
        cg::Matrix J(6,21);
        J.set_mat(0, 0, R_i_c);
        J.set_mat(0, 15, cg::Matrix::eye(3));
        J.set_mat(3, 0,  cg::skew_symmetric(R_w_i.transpose() * t_c_i));
        //J.block<3, 3>(3, 0) = -R_w_i.transpose()*skewSymmetric(t_c_i);
        J.set_mat(3, 12, cg::Matrix::eye(3));
        J.set_mat(3, 18, cg::Matrix::eye(3));
        ...
        const cg::Matrix &P11 = state_server.state_cov.block<21, 21>(0, 0);
        const cg::Matrix &P12 = state_server.state_cov.block(0, 21, 21, old_cols - 21);
...
//        state_server.state_cov.block(old_rows, 0, 6, old_cols) << J * P11, J * P12;
        ...
        cg::Matrix state_cov_fixed = (state_server.state_cov + state_server.state_cov.transpose()) / 2.0;
        state_server.state_cov = state_cov_fixed;
    ...
    void MsckfVio::addFeatureObservations(const CameraMeasurementConstPtr &msg) {
        ...
        for (const auto &feature : msg->features) {
            if (map_server.find(feature.id) == map_server.end()) {
                ...
                map_server[feature.id].observations[state_id] = cg::Vector4({feature.u0, feature.v0, feature.u1, feature.v1});
            } else {
    ...
    void MsckfVio::measurementJacobian(
            ...
            cg::Matrix& H_x, cg::Matrix& H_f, cg::Vector4 &r) {
        ...
        // Cam0 pose.
        cg::RotationMatrix R_w_c0 = cam_state.orientation.rotation_matrix();
        const cg::Vector3 &t_c0_w = cam_state.position;

        // Cam1 pose.
        cg::RotationMatrix R_c0_c1 = CAMState::T_cam0_cam1.rotation_matrix();
        cg::RotationMatrix R_w_c1  = CAMState::T_cam0_cam1.rotation_matrix() * R_w_c0;
        cg::Vector3 t_c1_w = t_c0_w - R_w_c1.transpose() * CAMState::T_cam0_cam1.translation();

        // 3d feature position in the world frame.
        // And its observation with the stereo cameras.
        const cg::Vector3 &p_w = feature.position;
        const cg::Vector4 &z = feature.observations.find(cam_state_id)->second;

        // Convert the feature position from the world frame to
        // the cam0 and cam1 frame.
        cg::Vector3 p_c0 = R_w_c0 * (p_w - t_c0_w);
        cg::Vector3 p_c1 = R_w_c1 * (p_w - t_c1_w);

        // Compute the Jacobians.
        cg::Matrix dz_dpc0(4,3);
        dz_dpc0(0, 0) = 1 / p_c0[2];
        dz_dpc0(1, 1) = 1 / p_c0[2];
        dz_dpc0(0, 2) = -p_c0[0] / (p_c0[2] * p_c0[2]);
        dz_dpc0(1, 2) = -p_c0[1] / (p_c0[2] * p_c0[2]);

        cg::Matrix dz_dpc1(4,3);
        dz_dpc1(2, 0) = 1 / p_c1[2];
        dz_dpc1(3, 1) = 1 / p_c1[2];
        dz_dpc1(2, 2) = -p_c1[0] / (p_c1[2] * p_c1[2]);
        dz_dpc1(3, 2) = -p_c1[1] / (p_c1[2] * p_c1[2]);

        cg::Matrix dpc0_dxc(3,6);
        dpc0_dxc.set_mat(0, 0, cg::skew_symmetric(p_c0));
        dpc0_dxc.set_mat(0, 3, -R_w_c0);

        cg::Matrix dpc1_dxc(3,6);
        dpc1_dxc.set_mat(0, 0, R_c0_c1 * cg::skew_symmetric(p_c0));
        dpc1_dxc.set_mat(0, 3, -R_w_c1);

        cg::RotationMatrix dpc0_dpg = R_w_c0;
        cg::RotationMatrix dpc1_dpg = R_w_c1;
        ...
        u.set_mat(3, 0, cg::skew_symmetric(p_w - cam_state.position_null) * IMUState::gravity);
        H_x = A - A * u * (u.transpose() * u).inv() * u.transpose();
        H_f = -const_cast<const cg::Matrix&>(H_x).block<4, 3>(0, 3);

        // Compute the residual.
        r = z - cg::Vector4({p_c0[0] / p_c0[2], p_c0[1] / p_c0[2], p_c1[0] / p_c1[2], p_c1[1] / p_c1[2]});
    ...
    void MsckfVio::featureJacobian(
        ...
        cg::Matrix H_xj(jacobian_row_size, 21 + state_server.cam_states.size() * 6);
        cg::Matrix H_fj(jacobian_row_size, 3);
        cg::VectorX r_j(jacobian_row_size);
        int stack_cntr = 0;
...
//        cg::Matrix A(cvU.rows, jacobian_row_size - 3);
...
//                A(i, j-j_start) = cvU.at<float>(i, j);
        ...
        cg::Matrix U1, W1, Vt1;
        cg::svd_fulluv(H_fj, W1, U1, Vt1);
        int j_start = U1.cols()-(jacobian_row_size-3);
        cg::Matrix A(U1.rows(), jacobian_row_size - 3);
...
//        }
    ...
    // TODO
    void MsckfVio::measurementUpdate(const cg::Matrix &H, const cg::VectorX &r) {
...
//            r_thin = Q1.transpose() * r;
        ...
        cg::Matrix H_thin(eH_thin.rows(), eH_thin.cols());
        for(int i=0; i<eH_thin.rows(); ++i)
            for(int j=0; j<eH_thin.cols(); ++j)
                H_thin(i,j) = eH_thin(i,j);
        cg::VectorX r_thin(er_thin.size());
        for(int i=0; i<er_thin.size(); ++i)
            r_thin[i] = er_thin(i);

        // Compute the Kalman gain.
        const cg::Matrix &P = state_server.state_cov;
        cg::Matrix S = H_thin * P * H_thin.transpose() + Feature::observation_noise * cg::Matrix::identity(H_thin.rows(), H_thin.rows());
        ...
        // TODO
        cg::Matrix K(eK.rows(), eK.cols());
        for(int i=0; i<eK.rows(); ++i)
            for(int j=0; j<eK.cols(); ++j)
                K(i,j) = eK(i,j);

        // Compute the error of the state.
        cg::VectorX delta_x = K * r_thin;

        // Update the IMU state.
        const cg::VectorX &delta_x_imu = delta_x.head<21>();
        ...
        const cg::Quarternion dq_imu = cg::Quarternion::small_angle_quaternion(delta_x_imu.head<3>());
        ...
        const cg::Quarternion dq_extrinsic = cg::Quarternion::small_angle_quaternion(delta_x_imu.segment<3>(15));
        ...
        for (int i = 0; i < state_server.cam_states.size(); ++i, ++cam_state_iter) {
            const cg::VectorX &delta_x_cam = delta_x.segment<6>(21 + i * 6);
            const cg::Quarternion dq_cam = cg::Quarternion::small_angle_quaternion(delta_x_cam.head<3>());
            cam_state_iter->second.orientation = dq_cam * cam_state_iter->second.orientation;
            cam_state_iter->second.position += delta_x_cam.tail<3>();
        }

        // Update state covariance.
        cg::Matrix I_KH = cg::Matrix::identity(K.rows(), H_thin.cols()) - K * H_thin;
    ...
    bool MsckfVio::gatingTest(const cg::Matrix &H, const cg::VectorX &r, const int &dof) {
        cg::Matrix P1 = H * state_server.state_cov * H.transpose();
        cg::Matrix P2 = Feature::observation_noise * cg::Matrix::eye(H.rows());
        cg::Matrix P = P1 + P2;
    ...
    void MsckfVio::removeLostFeatures() {
        ...
        cg::Matrix H_x(jacobian_row_size, 21 + 6 * state_server.cam_states.size());
        cg::VectorX r(jacobian_row_size);
        int stack_cntr = 0;
    ...
    void MsckfVio::findRedundantCamStates(std::vector<StateIDType> &rm_cam_state_ids) {
        ...
        const cg::Vector3 key_position = key_cam_state_iter->second.position;
        const cg::RotationMatrix key_rotation = key_cam_state_iter->second.orientation.rotation_matrix();

        // Mark the camera states to be removed based on the
        // motion between states.
        for (int i = 0; i < 2; ++i) {
            const cg::Vector3 position = cam_state_iter->second.position;
            const cg::RotationMatrix rotation = cam_state_iter->second.orientation.rotation_matrix();

            double distance = (position - key_position).l2norm();
            cg::RotationMatrix R = rotation * key_rotation.transpose();
    ...
    void MsckfVio::publish(double time_stamp) {
        ...
        cg::EuclideanTransform T_i_w;
        T_i_w.set_rotation_matrix(imu_state.orientation.rotation_matrix().transpose());
        T_i_w.set_translation(imu_state.position);

        cg::EuclideanTransform T_b_w = IMUState::T_imu_body * T_i_w * IMUState::T_imu_body.inv();

        // Publish the odometry: T_b_w
        cg::RotationMatrix m3_r = T_b_w.rotation_matrix();
        cg::Vector3 v3_t = T_b_w.translation();
        cg::Quarternion q4_r = m3_r.quarternion_hamilton(); // Hamilton for draw
        ...
        cg::Matrix P_oo = const_cast<const cg::Matrix&>(state_server.state_cov).block<3, 3>(0, 0);
        cg::Matrix P_op = const_cast<const cg::Matrix&>(state_server.state_cov).block<3, 3>(0, 12);
        cg::Matrix P_po = const_cast<const cg::Matrix&>(state_server.state_cov).block<3, 3>(12, 0);
        cg::Matrix P_pp = const_cast<const cg::Matrix&>(state_server.state_cov).block<3, 3>(12, 12);

        // TODO: verify
        cg::Matrix P_imu_pose(6,6);
        P_imu_pose.set_mat(0, 0, P_pp);
        P_imu_pose.set_mat(0, 3, P_po);
        P_imu_pose.set_mat(3, 0, P_op);
        P_imu_pose.set_mat(3, 3, P_oo);

        cg::Matrix H_pose(6,6);
        H_pose.set_mat(0, 0, IMUState::T_imu_body.rotation_matrix());
        H_pose.set_mat(3, 3, IMUState::T_imu_body.rotation_matrix());

        cg::Matrix P_body_pose = H_pose * P_imu_pose * H_pose.transpose();
        ...
        cg::Matrix P_imu_vel = const_cast<const cg::Matrix&>(state_server.state_cov).block<3, 3>(6, 6);
        cg::Matrix H_vel = IMUState::T_imu_body.rotation_matrix();
        cg::Matrix P_body_vel = H_vel * P_imu_vel * H_vel.transpose();
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j)
        ...
        for (const auto &item : map_server) {
            ...
            if (feature.is_initialized) {
                cg::Vector3 feature_position = IMUState::T_imu_body.rotation_matrix() * feature.position;
                points3d_.push_back(cg::Point3f(feature_position[0], feature_position[1], feature_position[2]));
            }
        }
